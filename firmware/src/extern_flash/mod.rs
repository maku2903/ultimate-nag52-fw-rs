use core::{cell::RefCell, cmp::{max, min}, mem::MaybeUninit, ops::DerefMut};

use atsamd_hal::{delay::Delay, ehal::delay::DelayNs, pac::Peripherals, prelude::_atsamd_hal_embedded_hal_digital_v2_OutputPin, qspi::{Command, OneShot, Qspi}, usb::UsbBus};
use bsp::LedExtFlash;
use embassy_sync::{blocking_mutex::{raw::CriticalSectionRawMutex, CriticalSectionMutex}, mutex::Mutex, once_lock::OnceLock};
use embassy_time::Timer;
use static_cell::StaticCell;
use usbd_storage::{subclass::scsi::{Scsi, ScsiCommand}, transport::{bbb::{BulkOnly, BulkOnlyError}, TransportError}};
...
impl ExternFlash {
    ...
    pub fn new(controller: Qspi<OneShot>, mut led: LedExtFlash) -> ExternFlash {
        ExternFlash {
            led,
            qspi: controller,
        }
    ...
    pub async fn init(&mut self) {
        ...
        self.qspi.set_clk_divider(2); // 100Mhz
        self.qspi.run_command(Command::WriteEnable).unwrap();
        self.qspi.write_command(Command::WriteStatus2, &[0x02]).unwrap();
        self.led.set_low().unwrap();
    }

    pub fn into_partitions(self) -> AllPartitions {
        static SHARED_FLASH: StaticCell<embassy_sync::mutex::Mutex<CriticalSectionRawMutex, ExternFlash>> = StaticCell::new();
    ...
    pub fn erase_region(&mut self, start_addr: u32, len: u32) {
        ...
        loop {
            ...
            unsafe {
                Peripherals::steal().wdt.clear().write(|w| w.clear().bits(0xA5));
            }
            if start % Self::E_BLK64_SIZE as u32 == 0 && left >= Self::E_BLK64_SIZE as u32 {
                ...
                if !self.check_erased(start, Self::E_SECTOR_SIZE as u32) {
                    // Not erased, we have to erase it
                    self.qspi.run_command(Command::WriteEnable).unwrap();
                    self.qspi.erase_command(Command::EraseSector, start).unwrap();
                    self.wait_ready();
                }
...
pub static FAT_PARTITION: OnceLock<Mutex<CriticalSectionRawMutex, (Partition, ScsiState)>> = OnceLock::new();
...
#[inline]
pub fn process_scsi_command(mut command: usbd_storage::subclass::Command<ScsiCommand, Scsi<BulkOnly<UsbBus, &mut [u8]>>>)
        ...
        match command.kind {
            ...
            ScsiCommand::Inquiry { .. } => {
                command.try_write_data_all(&[
                    ...
                    b'U', b'L', b'T', b'I', b'M', b'A', b'T', b'E', b' ', b'N', b'A', b'G', b'5', b'2',
                    b' ', b' ', // 16-byte product identification
            ...
            ScsiCommand::ReadCapacity10 { .. } => {
                ...
                let _ = &mut data[4..8].copy_from_slice(&u32::to_be_bytes(ExternFlash::E_SECTOR_SIZE as u32));
            ...
            ScsiCommand::ReadCapacity16 { .. } => {
                ...
                let _ = &mut data[8..12].copy_from_slice(&u32::to_be_bytes(ExternFlash::E_SECTOR_SIZE as u32));
                command.try_write_data_all(&data)?;
                command.pass();
            }
            ...
            ScsiCommand::Read { lba, len } => unsafe {
                ...
                if state.storage_offset != (len * ExternFlash::E_SECTOR_SIZE) {
                    ...
                    let end = (ExternFlash::E_SECTOR_SIZE * lba) as usize + (ExternFlash::E_SECTOR_SIZE * len) as usize;
                    ...
                    if let Err(e) = partition.try_read(start, &mut read[..max]) {
                        defmt::error!("QSPI Read failed {} Addr: {:08X}, len: {:08X}", e, start, max);
                        command.fail();
            ...
            ScsiCommand::Write { lba, len } => unsafe {
                ...
                if state.storage_offset != (len * ExternFlash::E_SECTOR_SIZE) {
                    if state.storage_offset == 0 {
                        // Erase region
                        if let Err(e) = partition.try_erase(lba * ExternFlash::E_SECTOR_SIZE, len * ExternFlash::E_SECTOR_SIZE) {
                            defmt::error!("Erase failed 0x{:08X} (0x{:08X} len): {}", lba * ExternFlash::E_SECTOR_SIZE, len * ExternFlash::E_SECTOR_SIZE, e);
                            command.fail();
                            return Err(TransportError::Error(BulkOnlyError::InvalidState))
                        }
                    }
                        ...
                        match command.read_data(&mut read) {
                            ...
                            Ok(count) => {
                                if let Err(e) = partition.try_write(start+total, &read[..count]) {
                                    defmt::error!("QSPI Write failed to addr {:08X} {} ({} bytes)", start+total, e, count);
                                    cmd_fail = true;
                                    break;
                                } else {
                                    total += count;
