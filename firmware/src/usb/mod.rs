...
use core::{ops::DerefMut, sync::atomic::AtomicI32};
...
use bsp::{hal::usb::usb_device::device::UsbRev, LedUsb, UsbDm, UsbDp};
use core::fmt::Write;
use embassy_sync::{
...
static USB: OnceLock<embassy_sync::mutex::Mutex<CriticalSectionRawMutex, UsbPeripherals<'static>>> =
    OnceLock::new();
...
fn poll_usb() {
    if let Some(mut lock) = USB.try_get().map(|x| x.try_lock().ok()).flatten() {
        ...
        if usb.dev.poll(&mut [&mut usb.scsi]) {
            //&mut usb.uart,
            usb.led.set_high().unwrap();
            if usb.dev.state() == UsbDeviceState::Default
                || usb.dev.state() == UsbDeviceState::Suspend
            {
                reset_scsi_state();
            }
...
pub fn usb_init<Gclk: GclkId>(
    ...
    let usb_alloc = USB_ALLOCATOR.init(UsbBusAllocator::new(bsp::native_usb(
        clock, usb, mclk, dp, dm,
    )));

    //let uart = SerialPort::new_with_interface_names(
    //    usb_alloc,
    //    Some("ULTIMATE-NAG52 COMM"),
    //    Some("ULTIMATE-NAG52 DATA"),
    //);
    let usb_scsi_buf = USB_TRANSPORT_BUF.init([0; 4096]);
    let scsi = usbd_storage::subclass::scsi::Scsi::new(
        usb_alloc,
        USB_PACKET_SIZE as u16,
        MAX_LUN,
        usb_scsi_buf.as_mut_slice(),
    )
    .unwrap();

    let ctrl_buf = USB_CTRL_BUF.init([0; 256]);

    let bus = UsbDeviceBuilder::new(usb_alloc, UsbVidPid(0x16c0, 0x27dd), ctrl_buf)
